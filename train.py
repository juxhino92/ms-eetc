...
class TrainModel():
    ...
    def __init__(self, sr0, sr1, sr2, rho=1, g=9.81, withPnBrake=True) -> None:
        ...
        curvatureRadius = ca.MX.sym('curvatureRadius')
        ds = ca.MX.sym('ds')

        p = ca.vertcat(gradient, curvatureRadius, ds)

        # ODE

        rollingResistance = sr0 + sr1*ca.sqrt(velocitySquared) + sr2*velocitySquared
        curvatureResistance = ca.if_else(curvatureRadius>=300, g*0.5/((curvatureRadius-30)*rho), ca.if_else(curvatureRadius>0, g*0.65/((curvatureRadius-55)*rho), 0))
        acceleration = traction + (pnBrake if withPnBrake else 0) - rollingResistance - g*gradient*(1/rho) - curvatureResistance
        ...
        self.accelerationFun = ca.Function('a', [x, u, gradient, curvatureRadius], [acceleration])
...
class TrainIntegrator():
    ...
    def solve(self, time, velocitySquared, ds, traction=0, pnBrake=0, gradient=0, curvatureRadius=0):
        ...
        p0 = ca.vertcat(gradient, curvatureRadius, ds)
    ...
    def calcLosses(self, velocity, dt, traction=0, pnBrake=0, gradient=0, curvatureRadius=0):

        mdl = self.model

        out = self.lossesIntegrator(ca.vertcat(velocity, 0, 0), ca.vertcat(traction, pnBrake if mdl.withPnBrake else [], gradient, curvatureRadius), dt)
    ...
    def calcRollingResistance(self, velocity, ds, traction=0, pnBrake=0, gradient=0, curvatureRadius=0):

        mdl = self.model

        out = self.rollingResistanceIntegrator(ca.vertcat(velocity**2, 0), ca.vertcat(traction, pnBrake if mdl.withPnBrake else [], gradient, curvatureRadius, ds), 1)
...
if __name__ == '__main__':
    ...
    solution = integrator.solve(t0, v0**2, ds, f0, gradient=gd, curvatureRadius=cr)
