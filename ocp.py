    ...
    def __init__(self, train, track, optsDict={}):
        ...
        for i in range(numIntervals+1):
            ...
            if i < numIntervals:
                ...
                # gradient and curvature radius of current index
                grad = self.points.iloc[i]['Gradient [permil]']/1e3
                curvatureRadius = self.points.iloc[i]['Curvature radius [m]']

                # acceleration constraints
                g += [trainModel.accelerationFun(ca.vertcat(time[i], velSq[i]), ca.vcat(u), grad, curvatureRadius)]
                ...
                out = trainIntegrator.solve(time=time[i], velocitySquared=velSq[i], ds=self.steps[i],
                    traction=Fel[i], pnBrake=Fpb[i], gradient=grad, curvatureRadius=curvatureRadius)
                ...
                if opts.energyOptimal:
                    ...
                    if not opts.integrateLosses:
                        ...
                        energyLossesTr, energyLossesRgb = trainIntegrator.calcLosses(ca.sqrt(velSq[i]), time[i+1]-time[i], Fel[i], Fpb[i], grad, curvatureRadius)
